...
def make_specify(parameters:dict, key:str, default:str, align_left:bool=False)->dict:
    ...
    if not val: 
        ...
        elem['class'] += ' specify'
    if align_left: elem['class'] += ' leftify'
    return elem

def make_choose(parameters:dict, key:str, default:str)->dict:
    val = parameters.get(key, None)
    if not val: return{'value':default,'class':'choose-var',}
...
def make_magic(
    ...
    default_blank:bool = True, # determines whether the return has 'empty' as a CSS class
    force_magic:bool=True, # other fn.s piggy backing off the detection here may not want magic for text, etc.
...
def make_location(parameters:dict, key:str, default:str, default_blank:bool=True):
    val = parameters.get(key, None)
    if not val: return{'value':default,'class':'magic',}
    ...
    try: # there are surely more ways to get a location name than this
        ...
        location_elem = make_magic(parameters, key, default, default_blank=default_blank, force_magic=True)
...
def get_inline(val: dict, magic_wrapper=True, ask_text:str='Ask Each Time') -> list:
    ...
    try:
        ...
        for attachment in attached:
            ...
            if not new_magic: new_magic = magic('VAR')
...
def image_crop(parameters) -> [dict]:
    position_elem = make_magic(parameters, 'WFImageCropPosition', 'Bottom Right', default_blank=False)
    coord_elems = []
    if position_elem['value'] in ['Centre', 'Top Left', 'Top Right', 'Bottom Left', 'Bottom Right']:
        ...
        position_elem['class'] = 'choose-var'
        coord_elems = [
            make_line('X Coordinate', make_specify(parameters, 'WFImageCropX', '0')), 
            make_line('Y Coordinate', make_specify(parameters, 'WFImageCropY', '0')), 
        ]
    line_elems = [
        make_line('Position', position_elem)
    ] + coord_elems + [
        make_line('Width', make_specify(parameters, 'WFImageCropWidth', '100')),
        make_line('Height', make_specify(parameters, 'WFImageCropHeight', '100')),
    ]
    return line_elems
...
def replace_ask(magic:dict,default:str)->dict:
    magic['value'] = {'Ask Each Time':default}.get(magic['value'],magic['value'])
    return magic

def product_details(parameters) -> [dict]:
    search_elem = make_magic(parameters, 'WFAttribute', 'All', default_blank=False)
    search_elem['class'] ='choose-var'
    line_elems = [make_line('Search By',search_elem)]
    
    if search_elem['value'] != 'Product ID':
        results_elem = make_magic(parameters, 'WFEntity', 'Songs', default_blank=False)
        results_elem['class'] = 'choose-var'
        line_elems += [make_line('Results', results_elem)]
    region_elem = make_magic(parameters, 'WFCountry', 'Region') # default region depends on user's location
...
def get_list(parameters) -> [dict]:
    ...
    for item in items:
        if isinstance(item, str):
            ...
            elems.append(get_inline(item['WFValue']['Value'], magic_wrapper=False))
    return elems
