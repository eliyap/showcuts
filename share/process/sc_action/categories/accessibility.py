...
class _base(action):
    ...
    def modify(self):
        # "1 / 0" -> "On / Off"
        debug = self.title
        on_off_elem = self.title[2]
        on_off_elem['value'] = {1:'on',0:'off'}.get(on_off_elem['value'],on_off_elem['value'])

        # title case the term
        self.title[0]['value'] = self.title[0]['value'].title()

        # hide on_off_elem if switch is not "Turn"
        if self.title[0]['value'] != 'Turn':
            on_off_elem['class'].append('hidden')
